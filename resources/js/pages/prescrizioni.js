document.addEventListener('DOMContentLoaded', () => {
    ...
    clearForm?.addEventListener('submit', e => {
        if (!confirm('Cancellare tutte le prescrizioni di questo paziente?')) e.preventDefault();
    });
    ...
    document.body.addEventListener('click', e => {
        if (e.target.matches('.add-slot-btn')) {
            const { day, step } = e.target.dataset;
            const list     = e.target.previousElementSibling;
            const template = document.getElementById('med-slot-template');
            const clone    = template.content.cloneNode(true);

            clone.querySelector('select').name = `schedule[${day}][${step}][medicines][]`;
            clone.querySelector('input').name  = `schedule[${day}][${step}][amounts][]`;
        ...
        if (e.target.matches('.slot-remove-btn')) {
            const row  = e.target.closest('.med-slot-row');
